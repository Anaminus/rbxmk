...
`define`  | table  | -            | Defines a number of variables to be used by the [preprocessor](#user-content-preprocess-filter). Each key-value pair in the given table is merged with the existing set of variables. Keys must be strings that are identifiers, and values can be either bools, numbers, strings, or Stringlike values.<br>Note that variables defined by `configure` cannot override variables defined by the [`--define`](USAGE.md#user-content-command-options) command option.

...
func ColorSequence() rbxmk.Reflector {
	return rbxmk.Reflector{
		Name:     rtypes.T_ColorSequence,
		PushTo:   rbxmk.PushTypeTo(rtypes.T_ColorSequence),
		PullFrom: rbxmk.PullTypeFrom(rtypes.T_ColorSequence),
		...
		Metatable: rbxmk.Metatable{
			"__tostring": func(s rbxmk.State) int {
				v := s.Pull(1, rtypes.T_ColorSequence).(types.ColorSequence)
				s.L.Push(lua.LString(v.String()))
				return 1
			},
			"__eq": func(s rbxmk.State) int {
				v := s.Pull(1, rtypes.T_ColorSequence).(types.ColorSequence)
				op := s.Pull(2, rtypes.T_ColorSequence).(types.ColorSequence)
				if len(op) != len(v) {
					s.L.Push(lua.LFalse)
					return 1
				}
		...
		Properties: rbxmk.Properties{
			"Keypoints": {
				Get: func(s rbxmk.State, v types.Value) int {
					...
					keypointRfl := s.MustReflector(rtypes.T_ColorSequenceKeypoint)
		...
		Constructors: rbxmk.Constructors{
			"new": {
				Func: func(s rbxmk.State) int {
					...
					switch s.Count() {
					case 1:
						switch c := s.PullAnyOf(1, rtypes.T_Color3, rtypes.T_Table).(type) {
						case types.Color3:
					...
					case 2:
						v = types.ColorSequence{
							types.ColorSequenceKeypoint{Time: 0, Value: s.Pull(1, rtypes.T_Color3).(types.Color3)},
							types.ColorSequenceKeypoint{Time: 1, Value: s.Pull(2, rtypes.T_Color3).(types.Color3)},
						}
					default:
				...
				Dump: func() dump.MultiFunction {
					return dump.MultiFunction{
						...
						{
							Parameters: dump.Parameters{
								{Name: "color0", Type: dt.Prim(rtypes.T_Color3)},
								{Name: "color1", Type: dt.Prim(rtypes.T_Color3)},
							},
							Returns: dump.Parameters{
								{Type: dt.Prim(rtypes.T_ColorSequence)},
							},

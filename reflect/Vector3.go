	...
	return Type{
		...
		Metatable: Metatable{
			...
			"__eq": func(s State) int {
				...
				return s.Push("bool", types.Bool(s.Pull(1, "Vector3").(types.Vector3) == op))
			},
			"__add": func(s State) int {
				op := s.Pull(2, "Vector3").(types.Vector3)
				return s.Push("Vector3", s.Pull(1, "Vector3").(types.Vector3).Add(op))
			},
			"__sub": func(s State) int {
				op := s.Pull(2, "Vector3").(types.Vector3)
				return s.Push("Vector3", s.Pull(1, "Vector3").(types.Vector3).Sub(op))
			},
			"__mul": func(s State) int {
				switch op := s.PullAnyOf(2, "number", "Vector3").(type) {
				case types.Double:
					return s.Push("Vector3", s.Pull(1, "Vector3").(types.Vector3).MulN(float64(op)))
				case types.Vector3:
					return s.Push("Vector3", s.Pull(1, "Vector3").(types.Vector3).Mul(op))
				default:
			...
			"__div": func(s State) int {
				switch op := s.PullAnyOf(2, "number", "Vector3").(type) {
				case types.Double:
					return s.Push("Vector3", s.Pull(1, "Vector3").(types.Vector3).DivN(float64(op)))
				case types.Vector3:
					return s.Push("Vector3", s.Pull(1, "Vector3").(types.Vector3).Div(op))
				default:
					s.L.ArgError(2, "attempt to multiply a Vector3 with an incompatible value type or nil")
					return 0
				}
			...
			"__unm": func(s State) int {
				return s.Push("Vector3", s.Pull(1, "Vector3").(types.Vector3).Neg())
			},
		},
